  ...
  switch (action.type) {
    case Clip2Frame: {
      if (!action.payload.frameIdx) {
        for (let idx = 1; idx < 5; idx++) {
          if (state.frame[idx]["src"]) continue;
          return {
            ...state,
            frame: {
              ...state.frame,
        ...
        return {
          ...
          clipList: {
            ...state.clipList,
            [dummyClip["clipIdx"]]: dummyClip["src"],
            [action.payload.clipIdx]: "",
          },
        };
      }
      return {
    ...
    case AddClip: {
      return {
        ...
        clipList: {
          ...state.clipList,
          [action.payload.idx]: action.payload.src,
        },
      };
    }
    case Frame2Clip: {
      if (state.frame[action.payload.frameIdx]["src"]) {
        return {
          ...
          clipList: {
            ...state.clipList,
            [state.frame[action.payload.frameIdx]["clipIdx"]]:
              state.frame[action.payload.frameIdx]["src"],
          },
          frame: {
            ...state.frame,
            [action.payload.frameIdx]: { clipIdx: 0, src: "" },
          },
        };
      }
      return state;
      ...
      if (state.frame[state.drag["endIdx"]]["src"]) {
        ...
        return {
          ...
          frame: {
            ...state.frame,
            [state.drag["startIdx"]]: {
              clipIdx: endClip.clipIdx,
              src: endClip.src,
            },
            [state.drag["endIdx"]]: { clipIdx: clip.clipIdx, src: clip.src },
          },
        };
      }
      return {
        ...
        frame: {
          ...state.frame,
          [state.drag["startIdx"]]: { clipIdx: 0, src: "" },
          [state.drag["endIdx"]]: { clipIdx: clip.clipIdx, src: clip.src },
        },
      };
    }
